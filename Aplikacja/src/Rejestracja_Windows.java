...
public class Rejestracja_Windows extends JFrame {
	...
	private JLabel jlNazwa_progrmu, jlWersja, jlPrawa, jlEmail, lblPodajImi, lblPodajNazwisko, lblPodajAdres,
			lblPodajNrTel, lblLogin, lblHaso;
	...
	public JTable JT_Tabela_uzytkowników;

	static String nazwa, UP;
	int numer;
	public static int wariant;
	...
	public static void main(String[] args) {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					Rejestracja_Windows rej = new Rejestracja_Windows(wariant, nazwa);
					rej.setVisible(true);
				} catch (Exception e) {
					e.printStackTrace();
				}
	 ...
	 * Create the frame.
	 */
	public Rejestracja_Windows(int wariant, String nazwa) throws SQLException {
		this.setTitle(nazwa);
		// this.setModal(true);
		...
		JB_OK.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent arg0) {
				...
				if (wariant == 1) {
					...
					try {
						JT_Tabela_uzytkowników = log.Edytuj(numer, imie, nazwisko, adres, email, numer_telfonu, login,
								haslo, uprawnienia);
				...
				if (wariant == 2) {
					...
					try {
						JT_Tabela_uzytkowników = log.Rejestracja(imie, nazwisko, adres, email, numer_telfonu, login,
								haslo);
				...
				if (poprawność == true) {
					...
					int numer = log.numer;
					String uprawnienia = log.uprawnienia;
					// System.err.println(numer + uprawnienia);
					AppUser user = null;
					try {
						try {
		...
		JB_Anuluj.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent arg0) {
				...
				setVisible(false);
				LogowanieWindow logowanie = null;
				try {
					logowanie = new LogowanieWindow();
				} catch (SQLException e) {
					// TODO Anuluj
					e.printStackTrace();
				}
